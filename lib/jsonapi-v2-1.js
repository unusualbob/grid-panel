  ...
  function jsonapi (args) {
    ...
    function connect() {
      ...
      _socket.on('end', function() {
        console.log('JSONAPI connection lost');
        connect();
      });
    }

    function openHandler () {
      console.log('JSONAPI connection estabilished');
      
      if (Object.keys(handlers).length > 0) {
        Object.keys(handlers).forEach(function(tag) {
          writeLine(handlers[tag].line);
        });
      }
    ...
    function jsonHandler (json) {
      if (typeof json.tag != "undefined" && handlers[json.tag])
        handlers[json.tag].callback(json);
      else
    ...
    function errorHandler (e) {
      if (e.toString().indexOf('ECONNREFUSED')) {
        console.log('Connection refused, retry...');
